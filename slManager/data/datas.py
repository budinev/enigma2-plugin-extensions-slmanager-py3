...
# --- INIZIO FIX CRASH ---
try:
    if not hasattr(config.plugins, "slmanager"):
        from Components.config import ConfigSubsection, ConfigText, ConfigNumber, ConfigYesNo, ConfigPassword, ConfigSelection
        config.plugins.slmanager = ConfigSubsection()
        config.plugins.slmanager.hostaddress = ConfigText(default='127.0.0.1', fixed_size=False)
        config.plugins.slmanager.port = ConfigNumber(default=15000)
        config.plugins.slmanager.user = ConfigText(default='admin', fixed_size=False)
        config.plugins.slmanager.passw = ConfigPassword(default='******', fixed_size=False)
