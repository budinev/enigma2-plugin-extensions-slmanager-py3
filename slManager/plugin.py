...
# Deve finire con P ed E maiuscole se lo hai caricato così
FTP_XML = 'http://webplusfeeds.sat-lodge.it/xml/PluginEmulators.xml'
FTP_CFG = 'http://webplusfeeds.sat-lodge.it/xml/cfg.txt'
...
class slManager(Screen):
    def __init__(self, session, args = False):
        ...
        self['actions'] = ActionMap(['OkCancelActions',
         ...
         'menu': self.configtv,
         'blue': self.Blue,
         'yellow': self.open_extra, # <--- CAMBIA SOLO QUESTA PAROLA
         'green': self.action,
         'info': self.CfgInfo,
         'red': self.stop}, -1)
    ...
    def open_extra(self):
        self.session.open(slDownloadPanel)
    def setBlueKey(self):
        if  self.currCam == 'no':
        ...
        try:
            ...
            if self.com.find('.ipk') != -1:
                if fileExists(destipk):
                    os.remove(destipk)              
                # Aggiunti apici '%s' per gli spazi e force-downgrade per il blocco
                os.system("wget %s -c '%s' -O %s > /dev/null" %(useragent, self.com, destipk))
                cmd0 = 'opkg install --force-overwrite --force-downgrade --force-depends ' + destipk
                
                # 2. Installa il file con i comandi di forzatura (evita il "refusing")
                cmd0 = 'opkg install --force-overwrite --force-downgrade --force-depends ' + destipk
                
                # 3. Apre la finestra nera (Console) per farti vedere l'installazione
                self.session.open(Console, title='IPK Installation', cmdlist=[cmd0, 'sleep 5'])
                
                # Modifichiamo cmd0 con le opzioni di forzatura totali
                cmd0 = 'opkg install --force-overwrite --force-downgrade --force-depends ' + destipk
                self.session.open(Console, title='IPK Installation', cmdlist=[cmd0, 'sleep 5'])
...
class slDownloadPanel(Screen):
    skin = """
    <screen name="slDownloadPanel" position="0,0" size="1920,1080" title="SatLodge Addon Panel" flags="wfNoBorder">
        <eLabel position="0,0" size="1920,1080" backgroundColor="#101010" zPosition="-1" />
        <widget name="title" position="60,50" size="1800,70" font="Regular;45" halign="center" transparent="1" foregroundColor="#ffffff" />
        <widget name="list" position="100,160" size="1720,740" itemHeight="60" scrollbarMode="showOnDemand" transparent="1" font="Regular;32" />
        <ePixmap pixmap="buttons/red.png" position="100,980" size="35,25" alphatest="on" />
        <widget name="key_red" position="150,970" size="300,45" font="Regular;30" halign="left" transparent="1" />
        <ePixmap pixmap="buttons/green.png" position="500,980" size="35,25" alphatest="on" />
        <widget name="key_green" position="550,970" size="300,45" font="Regular;30" halign="left" transparent="1" />
    </screen>
    """
    ...
    def __init__(self, session):
        ...
        self["title"] = Label(_("SatLodge Addon Panel"))
        self["key_red"] = Label(_("Chiudi"))
        self["key_green"] = Label(_("Installa"))
        self["actions"] = ActionMap(["OkCancelActions", "ColorActions"], {
            "ok": self.do_install,
            "cancel": self.close,
            "red": self.close,
            "green": self.do_install
        }, -1)
        
        # --- AGGIUNTO: Download automatico XML all'apertura ---
        # --- DOWNLOAD XML DALLA TUA REPO TEST ---
        try:
            import os
            # Usiamo le virgolette per l'URL per evitare errori di lettura
            cmd = 'wget --no-check-certificate --user-agent="Mozilla/5.0" "%s" -O %s' % (FTP_XML, resolveFilename(SCOPE_PLUGINS, "Extensions/slManager/PluginEmulators.xml"))
            os.system(cmd)
        except Exception as e:
            print("[slManager] Errore download: ", str(e))
            
        self.load_xml()

    def load_xml(self):
        import xml.etree.ElementTree as ET
        xml_file = resolveFilename(SCOPE_PLUGINS, "Extensions/slManager/PluginEmulators.xml")
        if os.path.exists(xml_file):
            try:
                ...
                for item in root.findall(".//plugin"):
                    name = item.get("name")
                    # Legge la categoria (cat), se non esiste mette "Vari"
                    category = item.get("cat") if item.get("cat") else "Vari"
                    
                    url_node = item.find("url")
                    if url_node is not None:
                        url = url_node.text.strip().replace('"', '')
                        # Creiamo un nome visuale che include la categoria
                        display_name = "[%s] %s" % (category.upper(), name)
                        self.list.append((display_name, url))
                
                # Ordiniamo la lista per categoria così sono tutti raggruppati
                self.list.sort(key=lambda x: x[0])
                self["list"].setList(self.list)
            except Exception as e:
                print("[slManager] Errore XML:", e)
    ...
    def do_install(self):
        ...
        if sel:
            name = sel[0]
            url = sel[1]
            cmd = 'rm -f /tmp/addon.ipk && wget --no-check-certificate --user-agent="Mozilla/5.0" "' + url + '" -O /tmp/addon.ipk && opkg install --force-overwrite --force-downgrade /tmp/addon.ipk'
            from Screens.Console import Console
            self.session.openWithCallback(self.install_finished, Console, title=_("Installazione %s") % name, cmdlist=[cmd])

    def install_finished(self, result=None):
        self.session.open(MessageBox, _("Installazione completata!"), MessageBox.TYPE_INFO, timeout=5)
def Plugins(**kwargs):
    iconpic = 'logo.png'
    if isFHD():
